...
import { OnboardingFormSchema, onboardingSchema } from "@/lib/schemas/onboarding";
import { zodResolver } from "@hookform/resolvers/zod";
import { Feather } from "@expo/vector-icons"
import React, { useState } from "react";
import { Controller, useForm } from "react-hook-form";
import { Image, KeyboardAvoidingView, TextInput, View, Text, TouchableOpacity } from "react-native";
...
export default function Onboarding() {
    ...
    const { control, handleSubmit, formState: { errors: formErrors }
    ...
    const handleSave = async ({ startingBalance }: OnboardingFormSchema) => {
        ...
        const { error: txError } = await authSupabase.from("transactions").
            insert({
                user_id: user!.id,
        ...
        <SafeAreaView className="flex-1 bg-brand-body" edges={["top"]}>
            ...
            >
                <View className="flex-1 px-6 justify-center -mt-16">
                    ...
                    <Text className="text-3xl font-bold text-[#1A1D26] mb-2">Let's get you set up</Text>
                    <Text className="text-brand-text-muted text-sm mb-10">A couple of quick details to personalise your experience.</Text>
                    <Text className="text-brand-bg text-sm font-medium mb-2">Starting balance</Text>

                    <View className="flex-row items-center bg-white border border-[#E8E6DF] rounded-xl px-4 py-3.5 mb-1">
                        <Text className="text-sm text-[#1A1D26] mr-2 leading-none">{selectedCurrency.symbol}</Text>
                        <Controller
                            control={control}
                            ...
                            render={({ field: { value, onChange } }) => {
                                return (
                                    <TextInput
                                        className="flex-1 text-sm font-bold text-[#1A1D26] p-0 m-0"
                    ...
                    <TouchableOpacity
                        ...
                        activeOpacity={0.85}
                    >
                        <Text className="text-white font-semibold text-sm">{saving ? "Saving..." : "Get started"}</Text>
                    </TouchableOpacity>
                </View>
            </KeyboardAvoidingView>
