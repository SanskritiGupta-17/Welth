...
export default function Home() {
    ...
    return (
        <SafeAreaView className='flex-1 bg-brand-bg' edges={["top"]} >
            ...
            <ScrollView className='flex-1 bg-brand-body' showsVerticalScrollIndicator={false}
                refreshControl={
            ...
            >
                <View className='bg-brand-bg rounded-b-[28px] px-5 pt-5 pb-[22px]'>
                    <View className='flex-row justify-between items-center mb-[22px]'>
                        ...
                        <View className='flex-row items-center gap-2.5'>
                            <View className='items-end'>
                                ...
                                <Text className='text-brand-text-primary text-base font-medium'>{user?.firstName ?? "there"}</Text>
                            </View>
                    ...
                    <View className='mb-[22px]'>
                        ...
                        <Text className='text-brand-text-primary text-[38px] font-medium tracking-tight'>{formatPrice(totalBalance, currency)}</Text>

                        <View className='flex-row gap-3.5 mt-2.5'>
                            <View className='flex-row items-center gap-1.5'>
                                <Feather name='arrow-down-right' size={14} color="#3DDC84" />
                                <Text className='text-brand-success text-[13px]'>{formatPrice(monthIncome, currency)}</Text>
                            </View>

                            <View className='flex-row items-center gap-1.5'>
                                <Feather name='arrow-down-right' size={14} color="#FF6B4A" />
                                <Text className='text-brand-coral text-[13px]'>{formatPrice(monthExpense, currency)}</Text>
                            </View>
                        </View>
                    </View>
                    ...
                    <View className='flex-row gap-2.5'>
                        {QUICK_ACTIONS.map((action) => (
                            ...
                            >
                                ...
                                <Text className='text-[#B8BAC2] text-[11px] font-medium text-center'>{action.label}</Text>
                            </TouchableOpacity>
                        ))}
                    </View>
                </View>
                ...
                <View className='px-5 pt-[18px] pb-5'>
                    ...
                    >
                        ...
                        {budget ? (
                            ...
                            <Text className='text-brand-text-secondary text-xs'>Tap to set a monthly spending budget</Text>
                        )}
                    </TouchableOpacity>

                    {expenseBreakdown.length > 0 && (
                        <View className='bg-white rounded-[18px] border border-[#E8E6DF] p-4 mb-[18px]'>
                            <Text className='text-[#1A1D26] text-sm font-medium mb-3'>Expense breakdown (this month)</Text>
                                    ...
                                    {expenseBreakdown.slice(0, 6).map((c) => (
                                        ...
                                        >
                                            <View className='flex-row items-center gap-1.5'>
                                                ...
                                                <Text className='text-brand-text-secondary text-[11px]'>{getCategoryConfig(c.category).label}</Text>
                                            </View>
                                            <Text className='text-brand-bg text-[11px] font-medium'>{formatPrice(c.amount, currency)}</Text>
                                        </View>
                                    ))}
                                </View>
                            </View>
                    ...
                    <View className='flex-row items-center justify-between mb-3'>
                        ...
                        <TouchableOpacity
                            onPress={() => router.push("/(root)/(tabs)/transaction")}
                        >
