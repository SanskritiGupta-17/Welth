    ...
    const filteredTransactions = useMemo(() => {
        ...
        return transactions.filter(
            (tx) =>
                ...
                tx.category.toLocaleLowerCase().includes(q),
        );
    }, [transactions, search]);
        ...
        <SafeAreaView className="flex-1 bg-brand-body" edges={["top"]}>
            <View className="px-5 pt-3 pb-2">
                <View className="flex-row justify-between items-center mb-3">
                    <Text className="text-xl text-brand-bg font-semibold">Transactions</Text>
                ...
                <View className="flex-row items-center gap-2 bg-white rounded-xl border border-[#E8E8DF] px-3.5 py-2.5 mb-2.5">
                ...
                <View className="flex-row gap-2 mb-2.5">
                    {FILTERS.map((filter) => (
                        ...
                        >
                            <Text className={`text-xs ${activeFilter === filter ? "text-white" : "text-brand-text-secondary"}`}>{filter}</Text>
                        </TouchableOpacity>
                    ))}
                </View>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                    <View className="flex-row gap-2">
                        ...
                        >
                            <Text className={`text-xs ${activeAccountId === null ? "text-white" : "text-brand-text-secondary"}`}>All Accounts</Text>
                        </TouchableOpacity>
                        ...
                        {accounts.map((account) => (
                            ...
                            >
                                <Text className={`text-xs ${activeAccountId === account.id ? "text-white" : "text-brand-text-secondary"}`}>{account.name}</Text>
                            </TouchableOpacity>
                        ))}
                    </View>
                </ScrollView>
            ...
            {loading ? (
                ...
                <View className="flex-1 items-center justify-center px-10">
                    ...
                    <Text className="text-brand-text-muted text-sm text-center mt-3">Couldn't load transactions.</Text>
                    <TouchableOpacity
                        onPress={() => loadData()}
                    ...
                    ListHeaderComponent={transactions.length > 0 ? (
                        <View className="bg-white rounded-2xl border border-[#E8E6DF] p-4 mb-4">
                            <View className="flex-row items-center justify-between mb-3">
                                <Text className="text-brand-bg text-xs font-medium">Daily income vs expense</Text>
                                <View className="flex-row items-center gap-3">
                                    <View className="flex-row items-center gap-1.5">
                                        <View className="w-2 h-2 rounded-full bg-brand-success" />
                                        <Text className="text-[10px] text-brand-text-secondary font-medium">Income</Text>
                                    </View>

                                    <View className="flex-row items-center gap-1.5">
                                        <View className="w-2 h-2 rounded-full bg-brand-coral" />
                                        <Text className="text-[10px] text-brand-text-secondary font-medium">Expense</Text>
                                    </View>
                                </View>
                            </View>
