...
Deno.serve(async () => {
    try {
        ...
        for (const budget of budgets ?? []) {
            ...
            try {
                ...
                if (userError) {
                    console.error(`Error fetching user ${budget.user_id}:`, userError);
                    continue;
                }
                ...
                if (expenseError) {
                    console.error(`Error fetching expenses for user ${budget.user_id}:`, expenseError);
                    continue;
                }
                ...
                const lastSentDate = budget.last_alert_sent ? new Date(budget.last_alert_sent) : null;
                const alertedThisMonth =
                    lastSentDate && !isNaN(lastSentDate.getTime()) && isSameMonth(lastSentDate, now)
    ...
    `);
                ...
                if (updateError) {
                    console.error(`Error updating budget alert state for budget ${budget.id}:`, updateError);
                }

                sent++;
            } catch (itemError) {
                console.error(`Error processing budget alert for user ${budget.user_id}:`, itemError);
            }
        }
        ...
        console.error("Budget alert service error:", err);
