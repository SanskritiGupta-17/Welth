...
export default function ReceiptScannerModal({
    ...
    const handleCapture = async () => {
        ...
        try {
            const photo = await cameraRef.current.takePictureAsync({
                base64: true,
                //0.6 quality is plenty for Gemini to read text of a receipt
                quality: 0.6,
            });
    ...
    const handlePickerFromLibrary = async () => {
            ...
            <View className="flex-1 bg-black">
                ...
                <SafeAreaView className=" absolute inset-0 " edges={["top", "bottom"]}>
                    <View className="flex-row items-center justify-between px-5 pt-3">
                        ...
                        <View className="flex-row items-center gap-1.5 bg-black/40 rounded-full px-3 py-1.5 mr-30">
                            <MaterialCommunityIcons
                                name="robot-outline"
                                size={12}
                                color={COLORS.teal}
                            />
                            <Text className="text-white text-[11px] font-medium">Align the receipt</Text>
                        </View>
                    </View>

                    <View className="flex-1" />

                    <View className="flex-row items-center justify-center px-10 pb-6">
                        <TouchableOpacity
                            onPress={handlePickerFromLibrary}
                            disabled={capturing}
                        ...
                        <TouchableOpacity
                            ...
                            activeOpacity={0.85}
                            className="w-[72px] h-[72px] rounded-full bg-brand-blue items-center justify-center" style={{ borderWidth: 3, borderColor: "rgba(255,255,255,0.85)" }}
                        >
                            {capturing ? (
                ...
                {!permission?.granted && permission?.canAskAgain === false && (
                    <View className="absolute inset-0 items-center justify-center bg-black/80 px-10">
                        ...
                        <Text className="text-white/70 text-sm mt-3 text-center">Camera access is off. Enable it in settings to scan receipts.</Text>
