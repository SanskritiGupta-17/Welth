...
export function CurrencyPicker({ visible, selectedCode, onSelect, onClose }: {
    ...
    const filtered = useMemo(() => {
        ...
        return ALL_CURRENCIES.filter(
            (c) => c.code.toLocaleLowerCase().includes(q) || c.name.toLowerCase().includes(q),
        );
    }, [search]);
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        ...
                        >
                            ...
                            <Text className="text-brand-bg text-sm font-medium w-12">{item.code}</Text>
                            <Text className="text-brand-text-secondary flex-1 text-sm" numberOfLines={1}>{item.name}</Text>
