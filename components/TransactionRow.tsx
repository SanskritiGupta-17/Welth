...
export default function TransactionRow({
    ...
    const row = (
        ...
        >
            <View className='w-10 h-10 rounded-full items-center justify-center mr-3' style={{ backgroundColor: `${config.color}22` }}>
                <Text className='text-lg'>{config.icon}</Text>
            </View>

            <View className='flex-1'>
                <Text className='text-brand-ng text-sm font-medium' numberOfLines={1}>{tx.description || config.label}</Text>
                ...
                <View className='flex-row items-center gap-1.5 mt-0.5'>
                    ...
                    <View className='px-1.5 py-0.5 rounded-full' style={{ backgroundColor: `${config.color}1A` }}>
                        <Text className='text-[10px] font-medium' style={{ color: config.color }}>{config.label}</Text>
                    </View>
